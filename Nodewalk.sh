...
if [ -z "$outputDirSam" ]; then outputDirSam="UPLOADS/NW_ST2"; fi

if [ -z "$reffa" ]; then reffa="Genomes/HG19_BDGP5_PhiX_K12.fa"; fi

if [ -z "$outputDirStats" ]; then outputDirStats="UPLOADS/STATS"; fi
...
#echo"${args[@]}"
echo $unqpre
echo "total number of files are " ; echo "$unqpre" | wc -w;
...
do
	...
	echo "Congradulations done with all analysis."
