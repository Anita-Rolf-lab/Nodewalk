while getopts "i:s:a:b:o:h:p:r:" option; do
  case $option in
    i ) inputDir=$OPTARG
    ;;
...
if [ -z "$baitseq" ]; then baitseq="GTCTATGTACTTGTGAATTATTTCACGT"; fi
...
do
	...
	if [ -f "$filep1" ]; then
		...
		python nodewalk_github_v1.py $f $renzyme $pdef $reffa $inputDir $outputDirSam $outputDirStats $baitseq
